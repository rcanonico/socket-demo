// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// ServerTCPmt - concorrente multi-task - v_2020 (Roberto Canonico)
// VERSIONE PER LINUX
// Sintassi: ServerTCPmt [port]
...
int main(int argc, char *argv[])
{
    ...
    int    num_proc = 0;              // numero di processi creati
 
    // Ignora il segnale di terminazione dei processi figlio
    ...
    while (true) {
        ...
        num_proc++;
        if (fork() == 0) {
            // Processo figlio: chiusura della socket in ascolto 
            close(sd);
            ...
            // Processo figlio: chiusura della connessione e terminazione
            printf("----------------------------------------------------------\n");
            close(nsd);
            exit(0);       
        }
        // Processo padre: chiusura della socket relativa alla connessione
