// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// ServerTCPmth - concorrente multi-thread - v_2020 (Roberto Canonico)
// VERSIONE PER LINUX
// Sintassi: ServerTCPmth [port]
//    port: numero di port TCP del server
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// Codice fornito solo a scopo illustrativo
// Non garantito esente da bug ... Sicuramente non thread-safe
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
int num_thread = 0;    // numero di thread attivi

// Thread per la gestione di una singola connessione
void *doit(void *vnsd)
{
    int nsd = *((int *)vnsd);         // copia locale del descrittore della socket
    ...
    char   reply[SERVER_BUFFER_SIZE]; // buffer per memorizzare la risposta
    int n_received;

    printf("Avviato thread....\n");
    while (1) {
        // Ricezione della request inviata dal client
        memset(request, 0, SERVER_BUFFER_SIZE);
        printf("Aspetto dati dal client....\n");
        n_received = read(nsd, &request, sizeof(SERVER_BUFFER_SIZE)-1);
        if (n_received == 0) {
            printf("Il client ha chiuso la connessione.\n");
            break;
        }
          
        printf("Thread %d - Ricevuto dal client %d bytes: <%s>\n", thread_id, n_received, request);
    ...
    // Thread: chiusura della connessione e terminazione
    printf("----------------------------------------------------------\n");
    close(nsd);
    printf("Il thread sta terminando...\n"); 
	num_thread--;
    return NULL;
}
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        // Controllo sul numero massimo di thread attivabili
        if (num_thread >= MAX_NUM_THREAD) {
            printf("Numero massimo di thread raggiunto (%d) - connessione rifiutata\n", num_thread);
            close(nsd);    // Chiusura forzata della nuova socket 
        } else {
            // Creazione di un thread per gestire la nuova connessione
            if (pthread_create(&hThr[++num_thread], NULL, doit, &nsd) < 0) {
                fprintf(stderr, "creazione thread fallita\n");
            }
			printf("Numero attuale di thread: %d\n", num_thread);
        }
    }        
